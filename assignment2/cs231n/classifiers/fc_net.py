...
#         print('mode: ',mode)
        ...
        dropout_caches = []
        for i in range(self.num_layers):
            #print(i,': ',self.params['W'+str(i+1)] is None)
            if i == self.num_layers-1:
#                 print('W1: ',self.params['W1'] is None)
                scores,cache = affine_forward(X_run,self.params['W'+str(i+1)],self.params['b'+str(i+1)])
            else:
#                 print(i)
                if self.normalization is None:
                    X_run,cache = affine_relu_forward(X_run, self.params['W'+str(i+1)], self.params['b'+str(i+1)])
                elif self.normalization == "batchnorm":
                    X_run,cache = affine_bn_relu_forward(X_run,self.params['W'+str(i+1)],self.params['b'+str(i+1)]
                                                    ,self.params['gamma'+str(i+1)],self.params['beta'+str(i+1)]
                                                    ,self.bn_params[i])
                elif self.normalization == "layernorm":
                    X_run,cache = affine_ln_relu_forward(X_run,self.params['W'+str(i+1)],self.params['b'+str(i+1)]
                                                    ,self.params['gamma'+str(i+1)],self.params['beta'+str(i+1)]
                                                    ,self.bn_params[i])
                else:
                ...
                if self.use_dropout:
                    X_run,dropout_cache = dropout_forward(X_run,self.dropout_param)

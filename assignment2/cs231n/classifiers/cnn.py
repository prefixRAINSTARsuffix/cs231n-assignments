...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        in_sz = num_filters * (1 + (input_dim[1] + (filter_size-1) // 2 * 2 - filter_size)) / 2 * (1 + (input_dim[2] + (filter_size-1) // 2 * 2 - filter_size)) / 2
        in_sz = int(in_sz)
        self.params['W2'] = np.random.randn(in_sz,hidden_dim) * weight_scale
